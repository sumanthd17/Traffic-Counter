...
public class MultiBoxTracker{
    ...
    DetectorActivity da = new DetectorActivity();
    ...
    public synchronized void draw(final Canvas canvas) {
        ...
        if(da.getTrafficLineCode() == 1 || da.getTrafficLineCode() == 2)
            canvas.drawLine(0,canvas.getHeight()*(0.4f),canvas.getWidth(),(canvas.getHeight()*0.4f),linePaint);
        else if(da.getTrafficLineCode() == 3 || da.getTrafficLineCode() == 4)
            canvas.drawLine(canvas.getWidth()*(0.5f), 0,canvas.getWidth()*(0.5f),canvas.getHeight(),linePaint);
        for(int i=0;i<points.size();i++) {
            ...
            for(int j=0;j<prevPoints.size();j++){
                if(prevPoints.get(j).id == points.get(i).id) {
                    points.get(i).lineStatusX = prevPoints.get(j).lineStatusX;
                    points.get(i).lineStatusY = prevPoints.get(j).lineStatusY;
                    if (da.getTrafficLineCode() == 1 || da.getTrafficLineCode() == 2) {
                        if (points.get(i).label.equals("bus") || points.get(i).label.equals("truck")) {
                            if (da.getTrafficLineCode()==2 && prevPoints.get(j).Y + (prevPoints.get(j).height / 4) < (canvas.getHeight() * 0.4f) && points.get(i).Y + (points.get(i).height / 4) > (canvas.getHeight() * 0.4f) && !points.get(i).lineStatusY) {
                                points.get(i).lineStatusY = true;
                                downCount++;
                            } else if (da.getTrafficLineCode()==1 && prevPoints.get(j).Y + (prevPoints.get(j).height / 4) > (canvas.getHeight() * 0.4f) && points.get(i).Y + (points.get(i).height / 4) < (canvas.getHeight() * 0.4f) && !points.get(i).lineStatusY) {
                                points.get(i).lineStatusY = true;
                                upCount++;
                            }
                        } else {
                            if (da.getTrafficLineCode()==2 && prevPoints.get(j).Y < (canvas.getHeight() * 0.4f) && points.get(i).Y > (canvas.getHeight() * 0.4f) && !points.get(i).lineStatusY) {
                                points.get(i).lineStatusY = true;
                                downCount++;
                            } else if (da.getTrafficLineCode()==1 && prevPoints.get(j).Y > (canvas.getHeight() * 0.4f) && points.get(i).Y < (canvas.getHeight() * 0.4f) && !points.get(i).lineStatusY) {
                                points.get(i).lineStatusY = true;
                                upCount++;
                            }
                        }
                        ...
                        if(da.getTrafficLineCode() == 3){
                            if(prevPoints.get(j).X > canvas.getWidth()*0.5f && points.get(i).X <= canvas.getWidth()*0.5f && !points.get(i).lineStatusX){
                                points.get(i).lineStatusX = true;
                                leftCount++;
                            }
                        }
                        else if(da.getTrafficLineCode() == 4){
                            if(prevPoints.get(j).X < canvas.getWidth()*0.5f && points.get(i).X <= canvas.getWidth()*0.5f && !points.get(i).lineStatusX){
                                points.get(i).lineStatusX = true;
                                leftCount++;
                            }
        ...
        if(da.getTrafficLineCode() == 0)
            canvas.drawText("Select a traffic type", 0, canvas.getHeight()*0.8f, linePaint);
        else if(da.getTrafficLineCode() == 1)
            canvas.drawText("Up Count : " + upCount, 0, canvas.getHeight()*0.8f, linePaint);
        else if(da.getTrafficLineCode() == 2)
            canvas.drawText("Down Count : " + downCount, 0, canvas.getHeight()*0.8f, linePaint);
        else if(da.getTrafficLineCode() == 3)
            canvas.drawText("Left Count : " + leftCount, 0, canvas.getHeight()*0.8f, linePaint);
        else
            canvas.drawText("Right Count : " + rightCount, 0, canvas.getHeight()*0.8f, linePaint);
    ...
    private void processResults(final List<Recognition> results) {
        ...
        currCentroids.clear();
        logger.i(String.valueOf(da.getTrafficLineCode()));
    ...
    private class Centroid {
        int id;
        float X, Y, height;
        boolean lineStatusX,lineStatusY;
        String label;
        Centroid(Float X, Float Y, String label, Float height) {
            this.id = -1;
            this.X = X;
            this.Y = Y;
            this.lineStatusX = false;
            this.lineStatusY = false;
            this.label = label;
            this.height = height;
        }
    }
